        ...
        return (
            ...
            + "\n".join(
                ...
                for i, name in enumerate(
                    ["poketes", "attacks", "types", "item", "effects"]
                )
            )
            + "\n"
        )
...
"""
            for i, typ in enumerate(sorted(types)):
                ...
                for j, poke in enumerate(
                    [
                        k
                        for k in sorted(list(pokes)[1:])
                        if pokes[k]["types"][0] == typ
                    ]
                ):
...
"""
            ...
            if pokete_type is not None:
                ...
                for poke in [
                    k
                    for k in sorted(list(pokes)[1:])
                    if pokes[k]["types"][0] == pokete_type
                ]:
    ...
    def poke_info(poke: str, multi_page: bool = False) -> str:
        ...
        if (evolve_pokete := pokes[poke]["evolve_poke"]) == "":
            ...
            }#{evolve_pokete.replace("_", "-")}) at level {
                pokes[poke]["evolve_lvl"]
            }"""
...
"""
    ...
    def attack_info(attack: str, multi_page: bool = False) -> str:
        ...
        eff = (
            None
            if attacks[attack]["effect"] is None
...
""" + "\n".join(
            Wiki.item_info(item=item, multi_page=multi_page)
...
"""
            out += str.join(
                "", [effect.ret_md() for effect in effect_list]
            ).replace("###", "##")
        else:
            out = """
## Effects
...
def gen(ex: str, options: list[str], flags: dict[str, list[str]]):
...
multi_flag = Flag(["--multi"], "Generates a folder `wiki` with the wiki files")
pics_flag = Flag(
