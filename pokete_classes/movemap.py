...
class Movemap(gm.GameSubmap):
    ...
    def __init__(self, height, width):
        super().__init__(
            obmp.ob_maps["playmap_1"], 0, 0, height=height, width=width, name="movemap"
        )
        self.name_label = se.Text("")
        self.balls_label = se.Text("")
        self.label_bg = se.Square(" ", self.width, 1, state="float")
        self.label = se.Text(
            "1: Deck  2: Exit  3: Map  4: Inv.  5: Dex  6: Clock  " "?: Help"
        )
    ...
    def assure_distance(self, x, y, width, height):
        ...
        for _c, i, j, _k in zip(
            [x, y], ["x", "y"], [self.width, self.height], [width, height]
        ):
            while _c - getattr(self, i) + _k >= j:
                self.set(
                    self.x + (1 if i == "x" else 0), self.y + (1 if i == "y" else 0)
                )
                self.show()
                time.sleep(0.045)
    ...
    def text(self, x, y, inp_arr):
        ...
        for text in arr:
            ...
            for i in range(len(text) + 1):
                self.multitext.outp(
                    liner(text[:i], self.width - (x - self.x + 1), "   ")
                )
                std_loop()
                if _ev.get() != "":
                    _ev.clear()
